...
app.use(Express.static(Path.join(__dirname, 'fitrition-site')));

app.get('/*', (_request, response) => {
  response.sendFile(Path.join(__dirname, 'fitrition-site', 'index.html'));
});

const port = process.env.PORT ? +process.env.PORT : 4200;
const host = process.env.HOST;
const callbackFn = (p: number) => {
  console.log(`Fitrition 🚀 app started on port ${p}`);
};
